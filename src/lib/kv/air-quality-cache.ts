...
const magazyn = (env: EnvWithBinding) => {
  const kv = env.AIR_KV
  if (!kv) {
    console.warn('[air-quality-cache.ts] Binding AIR_KV nie jest podłączony — operacja pominięta. Sprawdź wrangler.jsonc.')
    return null
  }
  return kv
}
...
export const get = async (env: EnvWithBinding, key: string): Promise<AirQualityCacheValue | null> => {
  ...
  if (!kv) return null
  try {
    return await kv.get<AirQualityCacheValue>(PREFIX + key, 'json')
  } catch (error) {
