...
const magazyn = (env: EnvWithBinding) => {
  const kv = env.ANALYTICS_BUFFER_KV
  if (!kv) {
    console.warn('[analytics-buffer.ts] Binding ANALYTICS_BUFFER_KV nie jest podłączony — operacja pominięta. Sprawdź wrangler.jsonc.')
    return null
  }
  return kv
}
...
export const get = async (env: EnvWithBinding, key: string): Promise<AnalyticsBufferValue | null> => {
  ...
  if (!kv) return null
  try {
    return await kv.get<AnalyticsBufferValue>(PREFIX + key, 'json')
  } catch (error) {
