...
const magazyn = (env: EnvWithBinding) => {
  const kv = env.NOTIFICATIONS_KV
  if (!kv) {
    console.warn('[notifications-queue.ts] Binding NOTIFICATIONS_KV nie jest podłączony — operacja pominięta. Sprawdź wrangler.jsonc.')
    return null
  }
  return kv
}
...
export const get = async (env: EnvWithBinding, key: string): Promise<NotificationsQueueValue | null> => {
  ...
  if (!kv) return null
  try {
    return await kv.get<NotificationsQueueValue>(PREFIX + key, 'json')
  } catch (error) {
