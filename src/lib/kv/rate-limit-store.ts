...
const magazyn = (env: EnvWithBinding) => {
  ...
  if (!kv) {
    console.warn('[rate-limit-store.ts] Binding RATE_LIMIT_KV nie jest podłączony — operacja pominięta. Sprawdź wrangler.jsonc.')
    return null
  }
  return kv
}
...
export const get = async (env: EnvWithBinding, key: string): Promise<RateLimitStoreValue | null> => {
  ...
  if (!kv) return null
  try {
    return await kv.get<RateLimitStoreValue>(PREFIX + key, 'json')
  } catch (error) {
