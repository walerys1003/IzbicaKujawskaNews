...
const magazyn = (env: EnvWithBinding) => {
  const kv = env.CAPTCHA_KV
  if (!kv) {
    console.warn('[captcha-tokens.ts] Binding CAPTCHA_KV nie jest podłączony — operacja pominięta. Sprawdź wrangler.jsonc.')
    return null
  }
  return kv
}
...
export const get = async (env: EnvWithBinding, key: string): Promise<CaptchaTokensValue | null> => {
  ...
  if (!kv) return null
  try {
    return await kv.get<CaptchaTokensValue>(PREFIX + key, 'json')
  } catch (error) {
