import { createR2Handler, generateKey } from './_base'

/**
 * Miniatury wideo
 *
 * Example:
 * ```ts
 * import { upload, get, getSignedUrl } from './video-thumbnails'
 *
 * const key = generateKey('videos/thumbnails', 'example.bin')
 * await upload(env.R2_VIDEO_THUMBNAILS, key, file, { contentType: 'application/octet-stream', filename: 'example.bin' })
 * const object = await get(env.R2_VIDEO_THUMBNAILS, key)
 * const signedUrl = await getSignedUrl(key, 3600, env.JWT_SECRET)
 * ```
 */
export const handler = createR2Handler({
  binding: 'R2_VIDEO_THUMBNAILS',
  prefix: 'videos/thumbnails',
  maxFileSize: 5242880,
  allowedMimeTypes: ['image/jpeg','image/png','image/webp'],
  defaultCacheControl: 'public, max-age=3600',
})

export const upload = handler.upload
export const get = handler.get
const removeObject = handler.delete
export { removeObject as delete }
export const list = handler.list
export const getSignedUrl = handler.getSignedUrl
export { generateKey }
